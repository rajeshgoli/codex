...
fn supervise_listener(
    ...
    loop {
        ...
        let exit = run_listener_generation(
            ...
            state,
            Arc::clone(&shutdown),
            Arc::clone(&generation_shutdown),
            Arc::clone(&active_workers),
        );

        generation_shutdown.store(true, Ordering::Release);
    ...
    while !shutdown.load(Ordering::Relaxed) {
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                let connection_shutdown = Arc::clone(&generation_shutdown);
                let worker_guard = ActiveWorkerGuard(Arc::clone(&active_workers));
...
#[cfg(unix)]
fn wait_for_workers(active_workers: &AtomicUsize) {
